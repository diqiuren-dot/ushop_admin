<template lang="">
  <el-dialog :title="obj.isadd?'添加菜单':'修改菜单'" :visible.sync="obj.isshow" @closed="cancel">
    <el-form
      :model="menuObj"
      label-width="100px"
    >
      ...
      <el-form-item label="上级菜单">
        <el-select v-model="menuObj.pid" placeholder="请选择菜单" @change="changePid">
          <el-option label="--请选择--" value="shanghai" disabled></el-option>
          <el-option label="顶级菜单" :value="0">顶级菜单</el-option>
          <el-option v-for='item in user' :key="item.id" :label="item.title" :value="item.id">
            {{item.title}}
          </el-option>
        </el-select>
      </el-form-item>
